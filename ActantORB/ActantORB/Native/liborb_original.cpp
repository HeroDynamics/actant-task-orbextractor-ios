#include "liborb_original.h"

using namespace cv;
using namespace std;

LibORBOriginal::LibORBOriginal(int max_feature_points)
{
    orb_extractor = new ORB_SLAM3_ORIGINAL::ORBextractorOriginal(max_feature_points, 1.5f, 4, 20, 7);
}

LibORBOriginal::~LibORBOriginal(void)
{
    if (orb_extractor) {
...
void LibORBOriginal::DetectAndCompute(void *y_plane_bytes,
                              int y_plane_width,
                              int y_plane_height,
                              ...
                              const char *compatibility_file_path)
{
    ...
    //                 Это гарантирует что в работе `ORBextractorOriginal::operator()` НЕ будет использоваться
    //                 'stereo fisheye' условие при расчете features points (descriptors) - а будет
    //                 использоваться ТОЛЬКО `monoIndex` (одна камера) - см. тело (реализацию) `ORBextractorOriginal::operator()`
    ...
    //                 примерно в 2 раза быстрее (~0.019 сек на кадр) чем это делает ORBExtractorOriginal (~0.036 сек на кадр).
